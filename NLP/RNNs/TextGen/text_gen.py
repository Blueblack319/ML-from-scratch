...
def generate_text(
    model: RNN, dataset: TextDataset, prediction_length: int = 100
) -> str:
    """
...
if __name__ == "__main__":
    data = open(
        "NLP/RNNs/TextGen/dataset/Book1.txt", "r"
    ).read()  # use any text file you want to learn
    data = data.lower()
    ...
    # Model
    rnnModel = RNN(
        1, hidden_size, len(text_dataset.chars)
    )  # 1 because we enter a single number/letter per step.
    # 왜 1? feature size는 embedding size아닌가?
